...
def get_args():
    ...
    parser.add_argument("--udec", default="sd", type=str)
...
def main_train(args, backbone, train_loader, test_loader, writer, cfg, device, udec, **kwargs): 
    ...
    if args.use_parallel:
        ...
        # use pretrained coco 34
        if args.model == "resnet34" and cfg.dataset == "coco":
            state_dict = torch.load("/home/wangxucong/L2D/save/coco34.pth") 
            backbone.load_state_dict(state_dict)
        if args.model == "resnet34" and cfg.dataset == "fli":
            state_dict = torch.load("/home/wangxucong/L2D/save/ud/fli-resnet34.pth") 
            backbone.load_state_dict(state_dict)
            
        if args.udec == "kd":
            kwargs["model_teacher"].load_state_dict(torch.load("/home/wangxucong/L2D/save/kd/swin-kd.pth"))
            print("kd loaded")
        
        
    # backbone.load_state_dict(torch.load("/home/wangxucong/L2D/save/baseline.pth"))
    ...
    optimizer_sp0 = torch.optim.SGD(lla, lr=lr, momentum=0.9, weight_decay=1e-4)  # torch.optim.Adam(backbone.parameters(), lr=cfg.lr,weight_decay=1e-4)
    ...
    # scheduler_sp0 = lr_scheduler.MultiStepLR(optimizer_sp0, milestones=[len(train_loader) *20 ,len(train_loader) *40 ,len(train_loader) *60],  last_epoch=-1, verbose=False)
    # scheduler_sp0 = lr_scheduler.OneCycleLR(optimizer_sp0, max_lr=cfg.lr, steps_per_epoch=len(train_loader), epochs=cfg.max_epoch, pct_start=0.2)

    step0 = Step0(backbone, optimizer_sp0, criterion, scheduler_sp0, writer, device, kwargs["datas"])
    if not cfg.evo:
        step0.learn(args, backbone, cfg.max_epoch, train_loader, test_loader, cfg, udec, **kwargs)
    else:
        with torch.no_grad():
            step0.visualize(args, backbone, cfg.max_epoch, train_loader, test_loader, cfg, udec, **kwargs)
...
def main(args, TIMESTAMP, device, datas):
    ...
    print(" %s =%s=> model:%s |%d %d"
          % (cfg.dataset,
             args.udec, args.model, cfg.img_size, cfg.batch_size))
    torch.cuda.empty_cache()

    setup_seed(0) 
    if ((args.local_rank==0 and args.use_parallel) or (not args.use_parallel)):
        writer = SummaryWriter(f"./log/{TIMESTAMP}_{args.udec}_{args.model}_{cfg.dataset}")
    else:
        writer = None
    ...
    main_train(args, model_student, train_loader, test_loader, writer, cfg, device, args.udec, model_teacher=model_teacher, alpha_t=0.8 ,datas=datas)
...
if __name__ == "__main__":
    ...
    file_handler = logging.FileHandler(f'./memo/{args.udec}_{TIMESTAMP}.log')
...
# scheduler_sp0 = lr_scheduler.OneCycleLR(optimizer_sp0, max_lr=cfg.lr, steps_per_epoch=len(train_loader), epochs=cfg.max_epoch, pct_start=0.2)
# scheduler_sp0 = lr_scheduler.MultiStepLR(optimizer_sp0, [40 * len(train_loader), 80 * len(train_loader)], gamma=0.1, last_epoch=-1)