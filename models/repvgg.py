...
pretrained_model_path = {
    "repvgg_a0": "/home/algroup/.cache/torch/hub/checkpoints/RepVGG-A0-train.pth",
    "repvgg_a2": "/home/algroup/.cache/torch/hub/checkpoints/RepVGG-A2-train.pth",
}
...
class RepVGG(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes=1000, width_multiplier=None, override_groups_map=None, deploy=False,
        ...
        self.gap = FastAvgPool2d(flatten=True)
    ...
    def forward(self, x, le: bool = False, ft: bool = False):
        ...
        out = self.gap(out)
        if ft:
            features = self.to_ft(out)  # make [bs, 2048, 7, 7] to [bs, 2048] for feature-based distillers
            if le:
                le, out = self.linear(out, le=True)
                return features, le, out
            else:
                out = self.linear(out)
                return features, out
        else:
...
def repvgg_a0(num_classes, pretrained=False):
    net = RepVGG(num_blocks=[2, 4, 14, 1], width_multiplier=[0.75, 0.75, 0.75, 2.5])
    if pretrained:
        net.load_state_dict(torch.load(pretrained_model_path["repvgg_a0"]))
    add_ml_decoder_head(net, num_classes=num_classes, num_of_groups=num_classes, decoder_embedding=768)
    return net


def repvgg_a2(num_classes, pretrained=False):
    net = RepVGG(num_blocks=[2, 4, 14, 1], width_multiplier=[1.5, 1.5, 1.5, 2.75])
    if pretrained:
        net.load_state_dict(torch.load(pretrained_model_path["repvgg_a2"]))
    add_ml_decoder_head(net, num_classes=num_classes, num_of_groups=num_classes, decoder_embedding=768)
    return net
