...
from data import VocDataset, CocoDataset, NUSWideDataset, FrierenDataset
import torch
...
def flickr(cfg, data_root, args):
    ...
    if not args.use_parallel:
        train_dataset = FrierenDataset(args, train_img_prefix, train_ann_file, class_name, img_size=cfg.img_size,
                                   train_mode=True)
        train_loader = DataLoader(train_dataset, batch_size=cfg.batch_size, shuffle=True,
                                  num_workers=8, drop_last=True, pin_memory=True)
        train_loader.num_classes = len(train_dataset.CLASSES)
        ...
        test_dataset = FrierenDataset(args, test_img_prefix, test_ann_file, class_name, img_size=cfg.img_size,
                                  train_mode=False)
        test_loader = DataLoader(test_dataset, batch_size=cfg.batch_size, shuffle=False, num_workers=8)
        test_loader.num_classes = len(test_dataset.CLASSES)
        print(">>> Test Dataloader Built!")
